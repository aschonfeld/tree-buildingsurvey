...
sub login_page {
	...
	$sth = $dbh->prepare("SELECT name FROM student_data WHERE name in (\"Schonfeld,Andrew\",\"Kiparsky,Jon\",\"Thelen,Glenn\",\"Bolker,Ethan\") ORDER BY name");
    $sth->execute();
...
sub load_survey {
	...
	print "    document.TreeApplet.setTree(\"$treeXML\");\n";
	print "}\n";
	print "</script>\n";
	print "</head>\n";
	...
	print "width=850 height=650 name=\"TreeApplet\">\n";
