	...
	public void paintComponent(Graphics2D g2) {
		...
		if(questionText.size() == 0){
			text = TBSGraphics.breakStringByLineWidth(g2,
					questionProps.getProperty(currentRadioQuestion.getQuestionKey()), getUnpaddedWidth());
			questionText = text;
		}else
			text = questionText;
		...
		drawText(text);
		incrementStringY();
		radioQuestionSelection = new Rectangle(getX() + TBSGraphics.padding.width, getStringY(),
				TBSGraphics.questionButtonsWidth, response.getQuestionCount() * (TBSGraphics.textHeight + TBSGraphics.padding.height));
		drawRadioSelectionButtons();
		drawRadio(radioText);
	}
	
	public void drawRadio(List<String[]> lines) {
		int questionX = getX() + TBSGraphics.questionButtonsWidth + (TBSGraphics.padding.width*2);
