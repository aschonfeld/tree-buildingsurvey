...
public class DropDownMenu<T> {
	...
	public void render(Graphics2D g2, int x, int width, Collection<String> labels){
		...
		int adjX = x + ((width/2) - ((maxLabelWidth + (TBSGraphics.padding.width*2) + TBSGraphics.textHeight + TBSGraphics.padding.height)/2));
		selectionArea = new Rectangle(adjX, y,
				maxLabelWidth + (TBSGraphics.padding.width*2),
				TBSGraphics.textHeight + TBSGraphics.padding.height);
		fireDropDownButton = new Rectangle(adjX + maxLabelWidth + (TBSGraphics.padding.width*2), y,
				TBSGraphics.textHeight + TBSGraphics.padding.height,
				TBSGraphics.textHeight + TBSGraphics.padding.height);
		entriesArea = new Rectangle(adjX, y + TBSGraphics.textHeight + TBSGraphics.padding.height,
				selectionArea.width + fireDropDownButton.width,
				elements.size() * (TBSGraphics.textHeight + TBSGraphics.padding.height));
		g2.setColor(Color.WHITE);
		g2.fill(selectionArea);
		if(renderSelection && selection != null)
			rowRenderer.renderRow(new Object[]{selection}, selectionArea, g2);
		else
			TBSGraphics.drawCenteredString(g2, "Select",
		...
		TBSGraphics.drawCenteredString(g2, "\u25BC",
				fireDropDownButton.x, fireDropDownButton.y,
				fireDropDownButton.width, fireDropDownButton.height);
		...
		if(displayDropDownMenu){
			...
			Rectangle temp = new Rectangle(entriesArea.x, entriesArea.y,
					entriesArea.width,TBSGraphics.textHeight + TBSGraphics.padding.height);
			for(T element : elements){
	...
	public void mousePressed(MouseEvent e){
		if(displayDropDownMenu){
			if(entriesArea.contains(e.getPoint())){
				...
				int index = (e.getY() - (entriesArea.y)) / (TBSGraphics.textHeight + TBSGraphics.padding.height);
