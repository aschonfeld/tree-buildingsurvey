...
public abstract class Prompt {
	...
	*/
	public boolean isOverButton(MouseEvent e){
		if(minimizedState)
			return closeButton.contains(e.getPoint()) || minimizeButton.contains(e.getPoint());
		else
			return bottomButtons.contains(e.getPoint()) || 
			(closeButton.contains(e.getPoint()) ||
	...
	*/
	 ...
	 */
	public void calculateValues(int lineCount, int extraHeight, boolean hasBottomButtons) {
		...
		if(renderClose){
			Rectangle temp = new Rectangle((anchorPoint.x + promptSize.width)-buttonHeight,
					anchorPoint.y, buttonHeight, buttonHeight);
			closeButton = temp;
			if(renderMinimize){
				minimizeButton = new Rectangle(temp.x - buttonHeight, temp.y, buttonHeight, buttonHeight);
	...
	public void drawBox() {
		...
		g2.draw(new Rectangle2D.Double(box.x-1.5, box.y-1.5, box.width+3, box.height+3));
	...
	public void drawResizedBox(){
		...
		Rectangle box = new Rectangle(pt.x-2, pt.y-2, dim.width+4, dim.height+4);
		g2.setColor(Color.lightGray);
		g2.fill(box);
		g2.setColor(Color.white);
		g2.setStroke(new BasicStroke(3));
	...
	public void drawMinimized(){
		if(minimizationInProgress || maximizationInProgress)
			...
			Dimension dim = new Dimension(textDim.width + (2 * (buttonHeight + TBSGraphics.padding.width)) + 3, buttonHeight+3);
			Point pt = new Point(model.getApplet().getWidth() - (dim.width + 1 + model.getView().getVerticalBar().getWidth()), model.getApplet().getHeight() - (buttonHeight + 5));
			...
			g2.draw(new Rectangle2D.Double(box.x-1.5, box.y-1.5, box.width+3, box.height+3));
			g2.setStroke(new BasicStroke());
			TBSGraphics.drawCenteredString(g2, minimizedTitle, pt.x + TBSGraphics.padding.width, pt.y, 0, buttonHeight+3);
			minimizeButton.setLocation(pt.x + (textDim.width + (2 * TBSGraphics.padding.width)), pt.y+1);
			drawUtilityButton(minimizeButton, 2);
			closeButton.setLocation(minimizeButton.x + buttonHeight, minimizeButton.y);
			drawUtilityButton(closeButton, 1);
		}
	}
